...
SQL:UpdateLastAccess
	...
	SET
		...
		USERS_LASTHOST = ?,
		USERS_LASTDATE = CURRENT TIMESTAMP
	WHERE
		USERS_IDX = ?
...
SQL:SelectUser
	SELECT
		...
		USERS_FWD_LIST,
		USERS_READDR,
		CHAR(DATE(USERS_ABSENT_SDATE), JIS )||' '||CHAR(DATE(USERS_ABSENT_SDATE), JIS) USERS_ABSENT_SDATE,
		CHAR(DATE(USERS_ABSENT_EDATE), JIS )||' '||CHAR(DATE(USERS_ABSENT_EDATE), JIS) USERS_ABSENT_EDATE,
		USERS_MAX_VOLUME,
		USERS_CUR_VOLUME,
		USERS_SPAM_LEVEL,
		USERS_ISOPEN
	...
	SELECT
		...
		A.USERS_FWD_LIST,
		A.USERS_READDR,
		CHAR(DATE(A.USERS_ABSENT_SDATE), JIS )||' '||CHAR(DATE(A.USERS_ABSENT_SDATE), JIS) USERS_ABSENT_SDATE,
		CHAR(DATE(A.USERS_ABSENT_EDATE), JIS )||' '||CHAR(DATE(A.USERS_ABSENT_EDATE), JIS) USERS_ABSENT_EDATE,
		A.USERS_MAX_VOLUME,
		A.USERS_CUR_VOLUME,
		A.USERS_SPAM_LEVEL,
		A.USERS_ISOPEN
	...
	WHERE
		...
		(
			...
			(
				B.ACCOUNT_ISDATED = 1
				AND
				ACCOUNT_SDATE <= CURRENT TIMESTAMP
				AND
				ACCOUNT_EDATE >= CURRENT TIMESTAMP
			)
		)
...
SQL:SelectAdminFilterList
	...
	WHERE
		FILTER_AUTH = ?
		AND
		SUBSTR(USERS_IDX,LOCATE('@', USERS_IDX)+1) = ?
